...
transformed data {
  vector[N] raw_proportions = to_vector(y) ./ to_vector(batch_size);
}
parameters {
...
model {
  ...
  sigma_y ~ normal(0, 1);
  if (likelihood){
   raw_proportions[ix_train] ~
     normal(p_train, sqrt(p_train .* (1-p_train) ./ to_vector(batch_size[ix_train]) + sigma_y^2));
  }
}
